...
std::optional<CanvasInfo> CanvasStore::create(int width, int height) {
  ...
  return createRgba(width, height, std::move(pixels));
}
...
bool CanvasStore::drawImage(CanvasHandle destination, std::uint32_t source,
  ...
  if (const auto* sourceSurface = lookup(source)) {
...
bool CanvasStore::release(CanvasHandle handle) {
  auto* surface = lookup(handle);
  if (!surface || !images_.release(surface->image)) return false;
  surface->image = 0;
  surface->width = 0;
  surface->height = 0;
...
void CanvasStore::uploadDirty() {
  for (auto& surface : surfaces_) {
    if (!surface.live || surface.dirtyX1 <= surface.dirtyX0 ||
